...
* Leverage the fact it is on a device with internet connectivity
  ...
  * This requirement will be weighted based on complexity and usefulness.
    * At minimum, your app should receive and display data from Cloud Firestore or an API.
    * There are various ways that you can meet this requirement at a more intermediate/advanced level. Some options include but are not limited to: allowing users to create data and send it to the backend (then displaying the results), connecting data to specific users and displaying it such that the data is clearly connected to specific users, allowing users to manipulate data after it has been created and then display, filter, sort, and use that data in a meaningful way.
...
* **Project Description**
  ...
  * If you have not already, include a section that specifies what types of data you plan on sending and/or receiving from an internet-based source such as the Firebase database (Cloud Firestore) or an API. If you are using a database, what data will your app utilize and store (e.g. user info, user history, favorites, camera photos, etc)? If you are using an API, what types of data and services will your app retrieve, display, and interact with? Will you be processing the data in any way (e.g. doing some statistical analysis, filtering or sorting the data, providing a way to search for more information on a specific input, etc)?
